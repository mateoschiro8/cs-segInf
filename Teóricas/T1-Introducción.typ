...
Las acciones que causan dicha violación son llamadas *ataques*. 

== Tipos de amenazas

=== Flujo normal de la información:
#align(center)[#image("img/FlujoNormal.png", width: 12cm)]

=== Interrupción del flujo de información:
#align(center)[#image("img/Interrupción.png", width: 12cm)]

- Destrucción, bloqueo o saturación del recurso. Amenaza a la *disponibilidad* de la información.

=== Intercepción de información:
#align(center)[#image("img/Intercepción.png", width: 12cm)]
...
#align(center)[#image("img/Alteración.png", width: 12cm)]

- Modificación del recurso. Amenaza a la *integridad* de la información.

=== Fabricación de información
#align(center)[#image("img/Fabricación.png", width: 12cm)]

- Incorporación de información, envío de mensajes falsos (*phishing*). Amenaza a la *autenticidad* de la información. 

=== Amenazas pasivas

El atacante *observa*, *recopila* o *analiza* información pero *no modifica* ni *interfiere* directamente con los sistemas o datos.  
Generalmente son muy difíciles de detectar y dependen del medio físico de transmisión.  
...
El atacante *interviene* en los sistemas o en la información, provocando alteraciones, interrupciones o insertando información.  
Son más simples de detectar, ya que los efectos son más visibles.  
...
Es un conjunto de reglas y objetivos que una organización define para proteger su información y sistemas. Define quién tiene acceso, cómo se manejan los datos.  

Un *mecanismo* de seguridad es un método, herramienta o procedimiento para hacer cumplir una política de seguridad.  
Los mecanismos pueden ser no técnicos, y se aplican mediante software, hardware u otros procesos. 
...
== Cuestiones operacionales y sociales
...
Si se los configura en forma inadecuada o se los usa incorrectamente, hasta los mejores controles de seguridad se vuelven inútiles o incluso peligrosos.  

Personal no entrenado puede ser una amenaza para la seguridad de un sistema.  
Además, el entrenamiento necesario no solamente es técnico.  
Muchos ataques exitosos provienen del uso de la *ingeniería social*.  
